...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None, is_causal = None):
        ...
        L, N, d_model = src.shape # L: seq/window  N: batch  d_model: d_model

        # 第一次卷积：输入 L*d_model，输出 1*d_model
        src_conv1 = self.conv1(src.permute(1, 2, 0))  # (N, d_model, L) -> (N, d_model, 1)
        src_conv1 = self.sigmoid1(src_conv1)  # (N, d_model, 1)

        # 第一次加权：逐行逐元素相乘
        # 将 src_conv1 的形状从 (N, d_model, 1) 扩展为 (N, d_model, L)，然后与 src 逐行逐元素相乘
        src_conv1_expanded = src_conv1.expand(-1, -1, L)  # (N, d_model, L)
        src_weighted1 = src * src_conv1_expanded.permute(2, 0, 1)  # (L, N, d_model)

        # 第二次卷积：输入 L*d_model，输出 L*1
        src_conv2 = self.conv2(src_weighted1.permute(1, 2, 0))  # (N, d_model, L) -> (N, 1, L)
        src_conv2 = self.sigmoid2(src_conv2)  # (N, 1, L)

        # 第二次加权：逐列逐元素相乘
        # 将 src_conv2 的形状从 (N, 1, L) 扩展为 (N, d_model, L)，然后与 src_weighted1 逐列逐元素相乘
        src_conv2_expanded = src_conv2.expand(-1, d_model, -1)  # (N, d_model, L)
        src = src_weighted1 * src_conv2_expanded.permute(2, 0, 1)  # (L, N, d_model)
